...
void USART1_IRQHandler(void) {
    ...
    if(USART1 -> ISR & USART_ISR_RXNE) insert_echo_char(USART1 -> RDR);
    if(USART1 -> ISR & USART_ISR_TXE) {
        if(fifo_empty(&output_fifo)) USART1 -> CR1 &= ~USART_CR1_TXEIE;
        else USART1 -> TDR = fifo_remove(&output_fifo);
    }
    //-----------------------------------------------------------------
...
static int getchar_irq(void) {
    if(line_mode) {
        while(!fifo_newline(&input_fifo)) asm("wfi");
    }
    return fifo_remove(&input_fifo);
...
static int putchar_irq(char ch) {
    while(fifo_full(&output_fifo)) asm("wfi");

    if(ch == '\n') fifo_insert(&output_fifo, '\r');
    else fifo_insert(&output_fifo, ch);
...
void tty_init(void) {
    ...
    USART1 -> CR1 &= ~(USART_CR1_M | (1 << 4*7) | USART_CR1_PCE);
    USART1 -> CR2 &= ~USART_CR2_STOP;
    USART1 -> CR1 &= ~USART_CR1_OVER8;
    USART1 -> BRR = 48000000 / 921600;
